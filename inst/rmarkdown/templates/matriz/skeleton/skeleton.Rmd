...
tabela_periodo <- 
  function (matriz, periodo, optativas = FALSE) {
    if (optativas) { # Filtrar unidades curriculares optativas
      ...
      caption <- 
        paste0("**Quadro ...** - Carga horária presencial teórica (P/T) e prática (P/P) e não presencial ",
               "teórica (NP/T) e prática (NP/P) das unidades curriculares (UC) optativas ofertadas no ",
               ifelse(length(periodo) == 1, paste0(nome_periodo[periodo], " período do "), ""), "Curso.")
      ...
      if (optativas) {
        caption <-
          paste0(
            ...
            ifelse(
              length(periodo) == 1, paste0(", ofertada no ", nome_periodo[periodo], " período do Curso."),
              "."))
      } else {
        caption <-
          paste0(
...
As unidades curriculares optativas ofertadas no Curso estão dispostas nos quadros abaixos.
...
tabela <- split(tabela[["Carga horária total da unidade curricular"]], tabela[["Área de conhecimento"]])
tabela <- sapply(tabela, sum)
png("carga-horaria-optativas-area-conhecimento.png", width = 480 * 2, height = 480 * 2, res = 72 * 2)
par(mar = c(5, 10, 4, 2) + 0.1)
barplot(tabela, horiz = TRUE, las = 1, xlab = "Horas")
dev.off()
knitr::include_graphics("carga-horaria-optativas-area-conhecimento.png")
```

```{r, results='asis'}
tabela_unidades_curriculares(matriz = matriz, periodo = c(1:10), optativas = TRUE)
```